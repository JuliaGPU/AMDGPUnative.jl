...
default_device(::DeviceRuntime) = HSARuntime.get_default_agent()

struct RuntimeQueue{Q}
    queue::Q
end
default_queue(device) = RuntimeQueue(default_queue(RUNTIME[], device))
default_queue(::DeviceRuntime, device) =
...
create_event(::DeviceRuntime) = HSASignal()
...
function create_executable(::DeviceRuntime, device, func)
    # link with ld.lld
...
create_kernel(::DeviceRuntime, device, exe, entry, args) =
...
launch_kernel(::DeviceRuntime, queue, kern, event;
